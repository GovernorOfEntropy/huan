...
**Note:** This is a whitepaper, not a HUAN card. `huan-compliant: false`. For the standard, see huan-spec v1.0.9.
...
The analyst (huan-analyst v1.2.0) scans the card graph for six tension types.
...
| 2 | huan-lifecycle v1.1.0 | Stale cards. Formal state machine from seed to pruned. Every transition gated. |
| 3 | huan-analyst v1.2.0 | Coherence drift. Six-tension scan. Structured reports. |
...
Not every artifact is a static file. The same perimeter extends to databases, event streams, and APIs. A database entry in the manifest declares its schema version — the analyst connects, queries, compares. Schema drift is Type 6 version skew. Connection failure is Type 2 stale reference. An event stream declares its schema registry and topics — the analyst checks compatibility. An API declares its contract version — the analyst validates reachability. The tension types are invariant. The check strategy adapts to the artifact type. Static files, live systems, streaming data — one perimeter, type-aware guards.
...
huan-surgeon v1.2.3 closes the loop the analyst opens. 
...
The pillars:
- **huan-spec v1.0.9** — atomic card format. The foundation.
- **huan-skill v1.0.4** — the HUAN product definition. This standard, managed.
- **huan-lifecycle v1.1.2** — state machine from seed to pruned. Deprecation propagation.
...
- **huan-analyst v1.2.2** — tension detection as computable graph property.
- **huan-surgeon v1.2.3** — agentic remediation. Surgical, calibrated, auditable.
...
- Source: huan-spec v1.0.9, huan-lifecycle v1.1.2, huan-analyst v1.2.2, huan-surgeon v1.2.3, huan-diagnostic v1.0.1, huan-visual v1.0.1, huan-h2r v1.1.0
