...
# huan-h2r v1.1.0 — Human-to-Role Retrieval Pipeline
...
- Audience contract enforcement — bidirectional gate at ingestion, graduated attention governance
...
## 5. Bidirectional Audience Contract

Every document is its own security boundary. The audience contract is bidirectional — both producer and consumer must declare:

| Side | Declaration | Location |
|---|---|---|
| Producer | `audience: {tag}` | Document frontmatter |
| Consumer | `audience: {tag}` | Agent identity / role definition |

The gate at ingestion: does what I am reading match who I am?

### Context Assembly with Audience

When assembling context for an agent role, the pipeline prepends audience declarations before body text:

1. **Prepend consumer audience.** The first tokens in context are `audience: {consumer-tag}` — the agent's identity.
2. **Prepend document audience.** For each document ingested, `audience: {doc-tag}` is prepended before body text.
3. **Alignment check.** Consumer tag vs. document tag. Match → full weight. Adjacent → high weight with declared dependency. Cross-boundary → discounted weight. Undeclared dependency → flagged violation.

This is not binary allow/deny. It is graduated attention governance — the system controls what dominates the context window, not what is excluded from it.

### Alignment Taxonomy

| Alignment | Relationship | Action |
|---|---|---|
| Exact match | Same audience tag | Full weight |
| Adjacent | Related domains (jepson/huan) | High weight, declare dependency in Irreducible Context |
| Cross-boundary | Unrelated domains (jepson/enterprise-executive) | Discounted weight |
| Undeclared dependency | Cross-boundary reference not in Irreducible Context | Flagged — audience boundary violation |

The first token the model processes shapes the entire inference. Prepending `audience: {tag}` before body text anchors transformer attention weights at the boundary. The model ingests "this is for engineers" before it ingests the content. Every document is a self-referential firewall — no external ACL, no permission server. The document carries its own gate.
...
- Audience contract always. Enforce bidirectional audience alignment on every retrieval. Prepending audience tags before body text is mandatory, not optional.
...
## 11. Irreducible Context

- **Source paths:** `docs/corpus/ZK-111-context-sprawl-is-the-new-monolith.md`, `docs/corpus/ZK-004-context-assembly.md`, `docs/corpus/ZK-073-agent-sprawl-coherence-layer.md`, `docs/huan-standard/huan-spec-1.0.9.md`
- **Dependencies:** huan-spec v1.0.9 (card format, status tiers, cross-references as graph edges), ZK-111 (context isolation principle), ZK-004 (context assembly pipeline), ZK-073 (agent sprawl — retrieval as coherence mechanism)
- **Versions referenced:** ZK-111 (huan, 2026-05-10), ZK-004 (huan, 2026-02-22), ZK-073 (huan, 2026-05-06), huan-spec v1.0.9 (2026-05-15)
